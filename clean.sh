...

{
	...
	# For on-device build cleanup AIDE app data directory shouldn't be erased.
	if [[ "$TERMUX_ON_DEVICE_BUILD" == "false" ]]; then
		...
		if [[ -e "$deletion_dir" ]]; then
			...
			# by current user, like the `builder` user in AIDE docker
