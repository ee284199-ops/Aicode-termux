...
TERMUX_PKG_DESCRIPTION="bionic libc, libicuuc, liblzma, zlib, and boringssl for package builder and AIDE-docker"
...
termux_step_get_source() {
	...
	[[ "$(git config --get user.name)" != '' ]] || git config --global user.name "AIDE Github Actions"
	[[ "$(git config --get user.email)" != '' ]] || git config --global user.email "contact@AIDE.dev"
...
termux_step_host_build() {
	...
	# /home/builder/.AIDE-build/AIDE-aosp/src/prebuilts/python/linux-x86/2.7.5/bin/python2:
