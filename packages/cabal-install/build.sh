...
TERMUX_PKG_MAINTAINER="Aditya Alok <alok@AIDE.org>"
...
# ```
# 	ghc-iserv: /home/builder/.AIDE-build/_cache/ghc-cross-9.12.1-i686-runtime/lib/ghc-9.12.1/lib/i386-linux-ghc-9.12.1-inplace/ghc-prim-0.13.0-inplace/libHSghc-prim-0.13.0-inplace.a: unhandled ELF relocation(Rel) type 10
...
termux_step_post_configure() {
	...
	if [[ "$TERMUX_ON_DEVICE_BUILD" == false ]]; then # We do not need iserv for on device builds.
		...
		cat <<-EOF >>cabal.project.local
			package *
			  ghc-options: -fexternal-interpreter -pgmi=$(command -v AIDE-ghc-iserv)
		EOF
	fi
