...
# shellcheck source=scripts/utils/AIDE/package/termux_package.sh
source "$TERMUX_SCRIPTDIR/scripts/utils/AIDE/package/termux_package.sh"
...
# Offline AIDE-packages bundle can be created by executing
...
TERMUX_BUILD_LOCK_FILE="${TMPDIR}/.AIDE-build.lck"
...
# Utility function to run binaries under AIDE environment via proot.
...
# Function to run AIDE-elf-cleaner during termux_step_massage
...
if [[ "$TERMUX_ON_DEVICE_BUILD" == "true" ]]; then
	...
	source "$TERMUX_PREFIX/bin/AIDE-setup-package-manager"
...
if [[ "${TERMUX_INSTALL_DEPS-false}" = "true" || "${TERMUX_PACKAGE_LIBRARY-bionic}" = "glibc" ]]; then
	...
	gpg --list-keys 2C7F29AE97891F6419A9E2CDB0076E490B71616B > /dev/null 2>&1 || {
		gpg --import "$TERMUX_SCRIPTDIR/packages/AIDE-keyring/grimler.gpg"
	...
	gpg --list-keys CC72CF8BA7DBFA0182877D045A897D96E57CF20C > /dev/null 2>&1 || {
		gpg --import "$TERMUX_SCRIPTDIR/packages/AIDE-keyring/AIDE-autobuilds.gpg"
		gpg --no-tty --command-file <(echo -e "trust\n5\ny") --edit-key CC72CF8BA7DBFA0182877D045A897D96E57CF20C
	}
	gpg --list-keys 998DE27318E867EA976BA877389CEED64573DFCA > /dev/null 2>&1 || {
		gpg --import "$TERMUX_SCRIPTDIR/packages/AIDE-keyring/AIDE-pacman.gpg"
