    ...
    public static TermuxSession execute(@NonNull final Context context, @NonNull ExecutionCommand executionCommand,
        ...
        if (executionCommand.executable == null) {
            if (!executionCommand.isFailsafe) {
                // Try bash/zsh first — the bootstrap "login" binary (termux-login)
                // hardcodes /data/data/com.termux/... paths internally, so it would
                // fail with ENOENT when our package name differs. bash/zsh are plain
                // ELF binaries that work correctly with LD_LIBRARY_PATH set.
                for (String shellBinary : new String[]{"bash", "zsh", "login"}) {
                    File shellFile = new File(defaultBinPath, shellBinary);
                    boolean exists = shellFile.exists();
                    boolean canExec = shellFile.canExecute();
                    Logger.logInfo(LOG_TAG, "shell probe: " + shellFile.getAbsolutePath()
                        + " exists=" + exists + " canExecute=" + canExec);
                    if (canExec) {
                        executionCommand.executable = shellFile.getAbsolutePath();
                        Logger.logInfo(LOG_TAG, "shell selected: " + executionCommand.executable);
                        break;
                    }
                }
            }
            ...
            if (executionCommand.executable == null) {
                ...
                Logger.logWarn(LOG_TAG, "no Termux shell found in " + defaultBinPath
        ...
        Logger.logInfo(LOG_TAG, "setupProcessArgs result: processArgs[0]=" + processArgs[0]
            + " (total=" + processArgs.length + ")");
        ...
        Logger.logInfo(LOG_TAG, "final exec: " + executionCommand.executable
            + " processName=" + processName + " isLoginShell=" + isLoginShell);
